...
public class GraphHandler {
    ...
    public synchronized List<RouteHint> showTree(AnimatablePanel panel) {
        ...
        animating = true;
        treeMap.clear();
        List<RouteHint> spider = buildTree(panel, treeNode(), -300.0, 600.0, -80.0, null, false);
        for (Node n : allNodes) {
    ...
    public synchronized List<RouteHint> updateTree(AnimatablePanel panel) {
        if (!animating) {
            cleanRebuiltTree();
            List<RouteHint> spider = buildTree(panel, treeNode(), -300.0, 600.0, -80.0, null, true);
            for (Node n : allNodes) {
                if (!n.inTree()) {
    ...
    private synchronized List<RouteHint> buildTree(AnimatablePanel panel, TreeNode treeNode, Double xStart, Double width, Double y, Node parent, boolean rebuilding) {
        ...
        synchronized (treeNode) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                allHints.addAll(buildTree(panel, treeNodeRep, xStart + (width * i), width, y + 80, node, rebuilding));
            }
